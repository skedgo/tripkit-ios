                           ...
                           firstDayOfWeek:(WeekdayIndex)firstDayOfWeek
{
  if (! recurrenceRule) {
    ...
    return NSLocalizedStringFromTableInBundle(@"Everyday", @"Shared", [TKStyleManager bundle], "Everyday in context of repetitions (especially recurring events)");
  } else if ([recurrenceRule isEqualToString:@"W0111110"]) {
    return NSLocalizedStringFromTableInBundle(@"Weekdays", @"Shared", [TKStyleManager bundle], "Every weekday in context of repetitions (especially recurring events)");
  } else if ([recurrenceRule isEqualToString:@"W1000001"]) {
    return NSLocalizedStringFromTableInBundle(@"Weekends", @"Shared", [TKStyleManager bundle], "Every weekend day in context of repetitions (especially recurring events)");
  }
  
  NSIndexSet *weekdays = [self daysOfWeek:recurrenceRule];
  if (weekdays.count == 1) {
    NSString *day = [self longStringForWeekday:[weekdays firstIndex]];
    return [NSString stringWithFormat:NSLocalizedStringFromTableInBundle(@"Every %@", @"Shared", [TKStyleManager bundle], "'Every %day' in context of repetitions (especially recurring events), e.g., 'Every Monday'. (old key: EveryDayFormat)"), day];
  }
...
#pragma unused(stop) // we need to go till the end
  ...
  return [NSString stringWithFormat:NSLocalizedStringFromTableInBundle(@"Every %@", @"Shared", [TKStyleManager bundle], @"'Every %day' in context of repetitions (especially recurring events), e.g., 'Every Monday'. (old key: EveryDayFormat)"), days];
}
...
+ (TKCustomEventRecurrenceRule *)recurrenceRuleFromEKRecurrenceRules:(NSArray *)recurrenceRules
...
+ (NSString *)humanReadableVersionOfEKRecurrenceRules:(NSArray *)recurrenceRules
{
  ...
  switch (rule.frequency) {
    case EKRecurrenceFrequencyDaily:
      frequency = NSLocalizedStringFromTableInBundle(@"Repeats daily", @"Shared", [TKStyleManager bundle], "Repeats daily");
      break;
      
    case EKRecurrenceFrequencyWeekly:
      frequency = NSLocalizedStringFromTableInBundle(@"Repeats weekly", @"Shared", [TKStyleManager bundle], "Repeats weekly");
      break;
      
    case EKRecurrenceFrequencyMonthly:
      frequency = NSLocalizedStringFromTableInBundle(@"Repeats monthly", @"Shared", [TKStyleManager bundle], "Repeats monthly");
      break;
      
    case EKRecurrenceFrequencyYearly:
      frequency = NSLocalizedStringFromTableInBundle(@"Repeats yearly", @"Shared", [TKStyleManager bundle], "Repeats yearly");
      break;
  }
  return frequency;
}

+ (EKRecurrenceRule *)EKRecurrenceRuleFromRecurrenceRule:(TKCustomEventRecurrenceRule *)rule
{
...
+ (TKCustomEventRecurrenceRule *)ruleWithRecurrence:(NSString *)recurrence
                                            endDate:(NSDate *)end
{
  if (recurrence) {
    TKCustomEventRecurrenceRule *rule = [[TKCustomEventRecurrenceRule alloc] init];
