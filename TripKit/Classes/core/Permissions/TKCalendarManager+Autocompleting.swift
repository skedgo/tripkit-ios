...
extension TKCalendarManager: SGAutocompletionDataProvider {
  
  public func autocompleteFast(_ string: String, for mapRect: MKMapRect) -> [TKAutocompletionResult] {
    let events = string.isEmpty ? fetchDefaultEvents() : fetchEvents(matching: string)
    return TKCalendarManager.autocompletionResults(for: events, search: string)
  }
  ...
  public func additionalActionString() -> String? {
    return isAuthorized() ? nil : NSLocalizedString("Include events", tableName: "Shared", bundle: TKStyleManager.bundle(), comment: "Button to include events in search, too.")
  }
