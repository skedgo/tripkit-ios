...
@interface TKFoursquareGeocoder ()
...
@implementation TKFoursquareGeocoder
...
- (TKAutocompletionResult *)autocompletionResultForAnnotation:(TKNamedCoordinate *)annotation
                                                   ...
                                                   nearRegion:(MKCoordinateRegion)coordinateRegion
{
  ...
  result.image = [TKStyleManager imageNamed:@"icon-search-poweredByFoursquare_36x36"];
  result.score    = [TKFoursquareGeocoder scoreForAnnotation:annotation forSearchTerm:inputString nearRegion:coordinateRegion];
  result.isInSupportedRegion = @([TKRegionManager.shared coordinateIsPartOfAnyRegion:annotation.coordinate]);

  return result;
}

+ (NSUInteger)scoreForAnnotation:(TKNamedCoordinate *)annotation
                   forSearchTerm:(NSString *)inputString
                      nearRegion:(MKCoordinateRegion)coordinateRegion
{
  ...
  NSUInteger subtitleScore = [TKAutocompletionResult scoreBasedOnNameMatchBetweenSearchTerm:inputString
                                                                                  candidate:[annotation subtitle]];
  
  if (titleScore == 0 && subtitleScore == 0) {
    [TKLog debug:@"TKFoursquareGeocoder" block:^NSString * {
  ...
  return [TKAutocompletionResult rangedScoreForScore:rawScore
            ...
            asAutocompletionResult:(BOOL)asAutocompletionResult
{
  ...
  for (NSDictionary *venueDict in venues) {
    ...
    TKNamedCoordinate *namedCoordinate = [[TKNamedCoordinate alloc] initWithLatitude:latitude
                                                                           longitude:longitude
                                                                                name:name
    ...
    NSUInteger score = [TKFoursquareGeocoder scoreForAnnotation:namedCoordinate
                                                  forSearchTerm:inputString
                                                     nearRegion:coordinateRegion];
    if (score == 0) {
      continue;
  ...
  if (asAutocompletionResult) {
    ...
    for (TKNamedCoordinate *coordinate in filtered) {
      TKAutocompletionResult *result = [self autocompletionResultForAnnotation:coordinate
                                                                 forSearchTerm:inputString
                                                                    nearRegion:coordinateRegion];
