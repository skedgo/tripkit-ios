...
extension DataAttachable {
  func decode<T>(_ type: T.Type, key: String) -> T? where T : Decodable {
    do {
      ...
      TKLog.info("Decoding of \(key) failed due to: \(error).")
      return nil
    }
  }
  ...
  func encode<T>(_ value: T?, key: String) where T : Encodable {
    do {
      ...
      TKLog.info("Encoding of \(String(describing: value)) to \(key) failed due to: \(error).")
    }
  }
  ...
  private var dataDictionary: NSMutableDictionary {
    get {
      ...
      do {
        ...
        TKLog.info("Decoding new data failed due to: \(error). Data: \(String(decoding: data, as: UTF8.self))")
    ...
    set {
      do {
        ...
        TKLog.info("Encoding new data failed due to: \(error). Dict: \(newValue)")
      }
    }
  }
}
