...
public class OAuthClient {
  ...
  private static func canHandle(_ url: URL) -> Bool {
    ...
    if let callback = TKConfig.shared.oauthCallbackURL(), url.absoluteString.hasPrefix(callback.absoluteString) {
      return true
  ...
  public func rx_initiate(forMode mode: String, form: BPKForm) -> Observable<OAuthResult> {
    ...
    guard let callbackURL = TKConfig.shared.oauthCallbackURL() else {
  ...
  private func handle(_ url: URL) throws -> Observable<OAuthResult>? {
    ...
    if let callbackURL = TKConfig.shared.oauthCallbackURL(), url.absoluteString.hasPrefix(callbackURL.absoluteString) {
...
//    print(pathURL)
  ...
  public static func clearFileCaches() {
    do {
      ...
      TKLog.error("OAuthClient", text: "Error clearing file caches: \(error)")
    }
  }
