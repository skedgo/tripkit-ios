...
NSString *const SGLocationManagerBackgroundUpdatesEnabled = @"SGLocationManagerBackgroundUpdatesEnabled";

NSString *const SGLocationManagerFoundLocationNotification =  @"kSGLocationManagerFoundLocationNotification";
...
@property (nonatomic, strong) SGKNamedCoordinate *currentLocationPlaceholder;
...
- (id<MKAnnotation>)currentLocationPlaceholder
{
  if (! _currentLocationPlaceholder) {
    _currentLocationPlaceholder = [[SGKNamedCoordinate alloc] initWithName:Loc.CurrentLocation address:nil];
  }
...
- (void)setBestLocationFix:(CLLocation *)bestLocationFix
{
	...
	if (bestLocationFix
		...
		for (SGLocationManagerLocationBlock subscribers in self.subscriberBlocks.allValues) {
...
- (void)performSuccess:(SGLocationManagerLocationBlock)success
						 orFailure:(SGLocationManagerFailureBlock)failure
{
	if (self.bestLocationFix) {
		success(self.bestLocationFix);
	} else if (failure) {
    NSDictionary *info = @{
                           NSLocalizedDescriptionKey: NSLocalizedStringFromTableInBundle(@"Could not determine your current location.", @"Shared", [SGStyleManager bundle], @"Error title when GPS failed."),
                           NSLocalizedRecoverySuggestionErrorKey: NSLocalizedStringFromTableInBundle(@"Please try again or set your location manually.", @"Shared", [SGStyleManager bundle], @"Error recovery suggestion when GPS fails."),
                           };
  ...
  if (newLocation.horizontalAccuracy < 500.0 && [self location:newLocation isInTheLastSeconds:60]) {
		...
		for (CLCircularRegion *aRegion in regionsToRaiseNotif) {
			...
			if ([narrowRegion containsCoordinate:newLocation.coordinate]) {
				...
				SGLocationManagerMonitorBlock block = [self.monitorBlocks objectForKey:aRegion.identifier];
