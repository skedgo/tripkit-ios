...
@interface TKUIPulsingAnnotationView ()
...
@implementation TKUIPulsingAnnotationView
...
- (void)rebuildLayers {
    if(self.shouldBeFlat) {
        ...
        [self.layer insertSublayer:self.colorHaloLayer atIndex:0];
		if (self.showDot) {
        ...
        [self.layer insertSublayer:self.glowingHaloLayer atIndex:0];
		if (self.showDot) {
...
- (UIImage*)haloImageWithRadius:(CGFloat)radius {
    ...
    UIImage *ringImage = [[TKUIPulsingAnnotationView cachedRingImages] objectForKey:key];
    ...
    if(!ringImage) {
        ...
        [[TKUIPulsingAnnotationView cachedRingImages] setObject:ringImage forKey:key];
